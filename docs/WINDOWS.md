...
- `desktop-quit.ps1`'s cleanup **ownership proof runs for real** on the `windows-latest` VM: a Pester suite ([`plugins/app-it-windows/tests/`](../plugins/app-it-windows/tests/desktop-quit.Tests.ps1)) spawns real processes and binds real loopback ports to confirm the quit stops a server whose recorded identity it can prove, and **leaves a recycled or foreign PID alone**. This is the first `.ps1` that actually *executes* in CI rather than only linting — but it exercises the decision logic headlessly. It is not a substitute for the interactive runtime seams below (a real dev-server tree, the Job-Object reap, taskbar identity), which still need a Windows session.
